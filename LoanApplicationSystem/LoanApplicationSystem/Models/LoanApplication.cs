...
namespace LoanApplicationSystem.Models
{
    public class LoanApplication : IValidatableObject
    {
        ...
        [RegularExpression(@"^\+\d{1,3}\s\d{3}(\s\d{3}){1,4}$",
    ErrorMessage = "Podaj poprawny numer telefonu, np. +48 222 000 222.")]
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            if (EmploymentType.HasValue && !Enum.IsDefined(typeof(EmploymentType), EmploymentType.Value))
            {
                yield return new ValidationResult(
                    "Wybrano nieprawidłowy rodzaj zatrudnienia.",
                    new[] { nameof(EmploymentType) });
            }

            if (CreditHistory.HasValue && !Enum.IsDefined(typeof(CreditHistory), CreditHistory.Value))
            {
                yield return new ValidationResult(
                    "Wybrano nieprawidłową historię kredytową.",
                    new[] { nameof(CreditHistory) });
            }

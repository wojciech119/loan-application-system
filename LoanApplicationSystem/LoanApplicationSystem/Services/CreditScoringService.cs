...
namespace LoanApplicationSystem.Services
{
    public class CreditScoringService
    {
        ...
        {
            if (application.EmploymentType == EmploymentType.BrakZatrudnienia)
            {
                return "Brak zdolności kredytowej";
            }

            if (application.CreditHistory == CreditHistory.Zla)
            {
                return "Brak zdolności kredytowej";
            }

            if (application.DisposableIncome < application.EstimatedMonthlyInstallment)
            {
                return "Brak zdolności kredytowej";
            }

            if (application.DtiPercent > 65)
            {
                return "Brak zdolności kredytowej";
            }
